...
A, X_input = [], []
for i in range(len(versions)):
    adj, adj_count, adj_win, feature = load_data_lstmgcn(DIR_NAME, i, versions)
...
#——————————————————train and predict—————————————————————
def train_gcn():
    model = R_rank_parameter(INPUT_SIZE, OUTPUT_SIZE, BATCH_SIZE, LR, REGULARIZATION_RATE, HIDDEN_SIZE, NODELAMBDA, RANKLAMBDA, COST_MATRIX, NUM_ADJ)
    ...
    for i in range(TRAIN_TIMES+1):
        ...
        while(start < TRAIN_END - 1):
            ...
            start += 1
        # if i % 50 == 0:
        #     print('%d cost: %f, %f' % (i, cost, 0))
    saver.save(sess, MODULE_FILE)

    # predict
    test_y = np.argmax(normalize_label[TRAIN_END], axis=1)
...
if __name__ == '__main__':
    ...
    for seed in range(20):
        ...
        df_measure.to_csv(DIR_NAME + '/' + 'R_SEED_' + str(RAND_SEED) + '.csv')
